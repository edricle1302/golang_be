...
func HandleSignup(w http.ResponseWriter, r *http.Request) {
	...
	// err = mysql_handler.Create_user(db, bodyStruct)
	// if err != nil && err.Error() == "email_exists" {
	// 	fmt.Println("Create user error:", err)
	// 	w.WriteHeader(http.StatusInternalServerError)
	// 	w.Header().Set("Content-Type", "application/json")
	// 	json.NewEncoder(w).Encode(map[string]interface{}{
	// 		"message":   "Signup email is already existed",
	// 		"exit_code": conf.GetInt("exit_code.email_exists"),
	// 	})
	// 	return
	// }
...
// 	err = cache.SetToRedis(ctx, email.(string)+"_forgot", token, 15000, redisClient)
// 	if err != nil {
// 		fmt.Println("Set to redis failed:", err)
// 		w.WriteHeader(http.StatusInternalServerError)
// 		w.Header().Set("Content-Type", "application/json")
...
// 		return
// 	}
// 	verification_url := "https://web.bachkhoi.online" + "/callback/forgotpassword?email=" + email.(string) + "&token=" + token
// 	err = cache.SetToRedis(ctx, email.(string)+"_verify", verification_url, 15000, redisClient)
// 	if err != nil {
// 		fmt.Println("Set to redis failed:", err)
// 		w.WriteHeader(http.StatusInternalServerError)
// 		w.Header().Set("Content-Type", "application/json")
...
// 		return
// 	}
// 	err = email_handler.SendEmail(email.(string), "Fogi password recovery", "forgot_email.html", map[string]interface{}{
// 		"VerificationCode": verification_url,
// 	})
// 	if err != nil {
// 		fmt.Println("Send verification email error:", err)
// 		w.WriteHeader(http.StatusInternalServerError)
// 		w.Header().Set("Content-Type", "application/json")
// 		json.NewEncoder(w).Encode(map[string]interface{}{
...
// func HandleCallbackForgotPassword(w http.ResponseWriter, r *http.Request) {
// 	fmt.Println("[+] HandleCallbackForgotPassword")
// 	bodyStruct, err := util.RequestToStruct(r)
// 	if err != nil {
// 		fmt.Println("Error parsing request body: ", err)
// 		w.WriteHeader(http.StatusInternalServerError)
...
// 	usrNewPassword, usrNewPasswordCheck := bodyStruct["password"]
// 	if !(emailCheck && usrTokenCheck && usrNewPasswordCheck) {
// 		w.WriteHeader(http.StatusInternalServerError)
// 		w.Header().Set("Content-Type", "application/json")
// 		fmt.Println(err)
// 		json.NewEncoder(w).Encode(map[string]interface{}{
...
// 		return
// 	}
// 	// email := fmt.Sprintf("%v", bodyStruct["email"])
// 	token, err := cache.GetFromRedis(ctx, email.(string)+"_forgot", redisClient)
// 	if err != nil {
// 		w.WriteHeader(http.StatusInternalServerError)
// 		w.Header().Set("Content-Type", "application/json")
// 		fmt.Println(err)
...
// 	db := mysql_handler.Init_database(os.Getenv("MYSQL_USER"), os.Getenv("MYSQL_PASSWORD"), os.Getenv("MYSQL_DB"))
// 	defer db.Close()
// 	verified, err := mysql_handler.Get_user_email_verified(db, email.(string))
// 	if err != nil {
// 		w.WriteHeader(http.StatusInternalServerError)
// 		w.Header().Set("Content-Type", "application/json")
// 		fmt.Println(err)
// 		json.NewEncoder(w).Encode(map[string]interface{}{
// 			"message":   "Error while taking user verified",
// 			"exit_code": conf.GetInt("exit_code.internal_error"),
// 		})
// 		return
// 	}
...
// 		return
// 	}
// 	err = mysql_handler.Update_user_password(db, email.(string), util.Hash_sha256(usrNewPassword.(string)))
// 	if err != nil {
// 		w.WriteHeader(http.StatusInternalServerError)
// 		w.Header().Set("Content-Type", "application/json")
// 		fmt.Println(err)
...
// 	usrToken := params.Get("token")
// 	if email == "" || usrToken == "" {
// 		w.Write([]byte("[+] Empty URL params field"))
// 		fmt.Println("[+] Empty URL params field")
// 	}
// 	token, err := cache.GetFromRedis(ctx, email+"_verify", redisClient)
// 	if err != nil {
// 		fmt.Println(err)
// 		w.WriteHeader(http.StatusUnauthorized)
// 		w.Header().Set("Content-Type", "application/json")
...
// 	db := mysql_handler.Init_database(os.Getenv("MYSQL_USER"), os.Getenv("MYSQL_PASSWORD"), os.Getenv("MYSQL_DB"))
// 	defer db.Close()
// 	mysql_handler.Set_verified_email_to_1(db, email)
// 	http.Redirect(w, r, "https://web.bachkhoi.online/verifysuccess", http.StatusSeeOther)
// }

// func HandleResendVerifyEmail(w http.ResponseWriter, r *http.Request) {
// 	bodyStruct, err := util.RequestToStruct(r)
// 	if err != nil {
// 		fmt.Println("Parse body error at resend verify email:", err)
// 		w.WriteHeader(http.StatusInternalServerError)
// 		w.Header().Set("Content-Type", "application/json")
// 		json.NewEncoder(w).Encode(map[string]interface{}{
// 			"message":   "Parse body failed",
...
// 	verification_code := util.GenerateSessionToken()
// 	err = cache.SetToRedis(ctx, email.(string)+"_verify", verification_code, 15000, redisClient)
// 	if err != nil {
// 		fmt.Println("Set to redis failed:", err)
// 		w.WriteHeader(http.StatusInternalServerError)
// 		w.Header().Set("Content-Type", "application/json")
...
// 		return
// 	}
// 	verification_url := "https://" + conf.GetString("be_const.domain") + "/verify/check?email=" + email.(string) + "&token=" + verification_code
// 	err = email_handler.SendEmail(email.(string), "Fogi verify email", "verify_email.html", map[string]interface{}{
...
// 			return
// 		}
// 		token := util.Generate6DigitsCode()
// 		err = cache.SetToRedis(ctx, email.(string)+"_forgot", token, 15000, redisClient)
// 		if err != nil {
// 			fmt.Println("Set to redis failed:", err)
// 			w.WriteHeader(http.StatusInternalServerError)
...
// 		}
// 		email := bodyStruct["email"]
// 		usrToken := bodyStruct["token"]
// 		usrOldPassword := bodyStruct["old_password"]
// 		usrNewPassword := bodyStruct["new_password"]
// 		required_fields := []interface{}{"email", "token", "old_password", "new_password"}
// 		for _, field := range required_fields {
// 			if _, ok := bodyStruct[field.(string)]; !ok {
// 				w.WriteHeader(http.StatusInternalServerError)
// 				w.Header().Set("Content-Type", "application/json")
// 				fmt.Println(err)
// 				json.NewEncoder(w).Encode(map[string]interface{}{
// 					"message":   field.(string) + " is required",
// 					"exit_code": conf.GetInt("exit_code.invalid_field"),
// 				})
// 				return
// 			}
...
// 			return
// 		}
// 		account, err := mysql_handler.Get_account(db, email.(string))
// 		if err != nil {
// 			fmt.Println(err)
// 			w.WriteHeader(http.StatusInternalServerError)
// 			w.Header().Set("Content-Type", "application/json")
// 			json.NewEncoder(w).Encode(map[string]interface{}{
// 				"message":   "Error while taking user account",
// 				"exit_code": conf.GetInt("exit_code.internal_error"),
// 			})
// 			return
// 		}

// 		if account["password"] != util.Hash_sha256(usrOldPassword.(string)) {
// 			w.WriteHeader(http.StatusUnauthorized)
// 			w.Header().Set("Content-Type", "application/json")
// 			json.NewEncoder(w).Encode(map[string]interface{}{
// 				"message":   "Wrong old password",
// 				"exit_code": conf.GetInt("exit_code.wrong_old_password"),
// 			})
// 			return
// 		}
...
func main() {
	...
	// http.HandleFunc("/forgot_password", RequestLogger(HandleForgotPassword))
	// http.HandleFunc("/callback/forgot_password", RequestLogger(HandleCallbackForgotPassword))
	// http.HandleFunc("/token/refresh", RequestLogger(HandleRefreshToken))
	// http.HandleFunc("/verify/send", RequestLogger(HandleResendVerifyEmail))
	// http.HandleFunc("/verify/check", RequestLogger(HandleVerifyEmail))
	// http.HandleFunc("/password", RequestLogger(HandlePassword))
